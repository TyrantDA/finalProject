...
backWheels::backWheels(float xx, float zz)
{
	pos.x = xx;
	pos.y = 1650;
	pos.z = zz;
...
void backWheels::loadObj()
{
	cout << " loading model " << endl;
	if (objLoader.loadModel("TestModels/wheels.obj", model))//returns true if the model is loaded, puts the model in the model parameter
	{
...
void backWheels::projection(glm::mat4 matrix)
{
	ProjectionMatrix = matrix;
}

void backWheels::shader(Shader* shader)
{
	myShader = shader;
}

glm::vec3 backWheels::getPos()
{
	return pos;
}

glm::quat backWheels::getRot()
{
	return rot;
}

void backWheels::update(glm::quat l, glm::quat r, glm::quat ro, glm::quat so, glm::vec3 p)
{
